...
int LocalConfigInfo::Load(const char *path) {
  ...
  memcpy(tips_info_.sub_stage_end_, tips["sub_stage_end"].asString().c_str(), sizeof(tips_info_.sub_stage_end_));
   CSJson::Value tips_arry = root_["loading_tips"];
  SetLoadingTipsInfo(tips_arry[0], 0);
  SetLoadingTipsInfo(tips_arry[1], 1);
  SetLoadingTipsInfo(tips_arry[2], 2);
  SetLoadingTipsInfo(tips_arry[3], 3);
